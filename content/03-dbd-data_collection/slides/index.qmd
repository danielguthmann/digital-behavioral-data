...
| [2]{.rn rn-type="strike-through" rn-color="#E6002E"} | [02.11.2022]{.rn rn-type="strike-through" rn-color="#E6002E"} | [DBD: Einführung und Überblick]{.rn rn-type="strike-through" rn-color="#E6002E"} | [Christoph Adrian]{.rn rn-type="highlight"}                         |
...
graph_1 <- svgtools::read_svg("graphs.svg")
graph_1_string <- graph_1 %>% 
  toString() %>% 
  stringr::str_replace("label_box_1", "Population") %>% 
  stringr::str_replace("User", "") %>% 
  # delete lines
  stringr::str_replace('style="fill:#ff0000;stroke:#ff0000"', 'style="fill:#ffffff00;stroke:#ffffff00"')


# Farbe Kopf: 00ff00ff
# Farbe kontur Box: ffffffff
# Farbe bulletpoint: d4aa00ff



output1 <- svgtools::read_svg(graph_1_string)
svgtools::display_svg(output1)
```
...
graph_base <- svgtools::read_svg("graph_base.svg")
graph_test <- graph_base %>% 
  toString() %>%
  # test transformation
  stringr::str_replace("label_box_1", "Population") %>% 
  # Group: Construct (HIDDEN)
  stringr::str_replace_all('"#FFFF00"', '"#FFFF0000"') %>% # Symbol
  stringr::str_replace_all('label_box_2', '') %>%  # Text
  # Group: Survey
  stringr::str_replace_all('"#70AD47"', '"#70AD4700"') %>% # Symbol %>% # Symbol & Lines
  stringr::str_replace_all('label_box_3', '') %>%   # Text
  # Group: SNS Fields
  stringr::str_replace_all('"#C55A11"', '"#C55A1100"') %>% # Stroke boxes
  stringr::str_replace_all('"#7F7F7F"', '"#7F7F7F00"') %>% # Cirlces filling
  stringr::str_replace_all('stroke="#04316A"',
                           'stroke="#04316A00"') %>% # Circles Strokes
  stringr::str_replace_all('field', '') %>%  # Text
  # Group: SNS Fields Textboxes
  stringr::str_replace_all('"#7030A0"', '"#7030A000"') %>% # Strokes & Filling boxes
  ## Box 1: Texts
  stringr::str_replace_all('style="fill:#ff00ff"', 'style="fill:#ff00ff00"') %>% # Bulletpoints 
  stringr::str_replace_all('box_1_h1', '') 
...
graph_test %>% 
  svgtools::read_svg(.) %>% 
  svgtools::display_svg(.)

  ## Text
  str
  # symbol & lines survey
  # # hide logo 
  # stringr::str_replace('fill=\"#04316A\" id=\"path257\" inkscape:label=\"logo_twitter\"/>',
  #                      'fill=\"#04316A00\" id=\"path257\" inkscape:label=\"logo_twitter\"/>')




graph_base %>% svgtools::display_svg()
...
graph_3 <- svgtools::read_svg("graphs3.svg")
graph_3_string <- graph_3 %>% 
  toString() %>% 
  stringr::str_replace('style="display:inline;fill:#00ff00" inkscape:label="symbol_head_container"',
                       'style="display:inline;fill:#e6002e" inkscape:label="symbol_head_container"')

graph_3_stringb <- graph_3 %>% 
  toString() %>% 
  stringr::str_replace('style=\"display:inline;fill:#00ff00\" inkscape:label=\"symbol_head_container\""',
                       'style=\"display:inline;fill:#e6002e\" inkscape:label=\"symbol_head_container\"')

graph_3_stringc <- graph_3 %>% 
  toString() %>% 
  stringr::str_replace("label_box_1", "Population") %>% 
  stringr::str_replace('style="stroke:#ff6600;fill:#ff6600', 'style="stroke:#ff660000;fill:#ff660000')

graph_3_stringd <- graph_3 %>% 
  toString() %>% 
  stringr::str_replace("label_box_1", "Population") %>% 
  stringr::str_replace('#ff6600"','#ff660000"')
...
output3 <- svgtools::read_svg(graph_3_stringc)
svgtools::display_svg(output3)
```
